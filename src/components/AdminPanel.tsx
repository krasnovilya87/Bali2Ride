...
export const AdminPanel = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-background w-full max-w-6xl h-full md:h-auto md:max-h-[90vh] md:rounded-[40px] overflow-hidden shadow-2xl border border-border flex flex-col"
      >
        {/* Header */}
        <div className="px-8 py-6 border-b border-border flex items-center justify-between bg-surface/30 backdrop-blur-md sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            {authLoading ? (
              ...
              <div className="flex items-center gap-3 bg-muted/30 px-3 py-1.5 rounded-2xl border border-border/50">
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg text-xs font-bold transition-all cursor-pointer"
                >
            ...
            <button 
              onClick={onClose} 
              className="p-3 hover:bg-muted rounded-full transition-colors active:scale-90 cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
